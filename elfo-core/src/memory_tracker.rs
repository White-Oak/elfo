...
pub(crate) struct MemoryTracker {
    /// A threshold that available memory should not reach (in fractions of total memory).
...
impl MemoryTracker {
    ...
    /// Returns `Ok(false)` if available memory amount is below a threshold.
